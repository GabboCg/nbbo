...
R implementation of the risk information measure from Smith & So (2021), adapted for **Analyst/Investor Day** events instead of earnings announcements.
...
riskinfo/
├── data/
│   └── sample_output.csv      # sample rows of the final output
├── R/
...
| 4 | Link Compustat → CRSP via `crsp.ccmxpf_linktable`; add S&P 500 membership flag and NYSE size decile |
...
| `crsp.ccmxpf_linktable`, `crsp.dsf`, `crsp.dsi`, `crsp.msp500list`, `crsp.mport1` | CRSP daily stock and index data |
...
`smithsoJAR.csv` — panel of event-firm observations with identifiers and risk-information measures.
