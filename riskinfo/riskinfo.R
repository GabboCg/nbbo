...
crsp_daily <- dbGetQuery(wrds, sprintf("
    SELECT s.permno, s.date,
           s.ret, s.vol, s.shrout, s.prc, s.ask, s.bid,
           i.ewretd
    FROM crsp.dsf s
    JOIN crsp.dsi i ON s.date = i.date
...
# ── Export ────────────────────────────────────────────────────────────────────
write.csv(smithso_jar, "smithsoJAR.csv", row.names = FALSE)
cat("Exported", nrow(smithso_jar), "observations to smithsoJAR.csv\n")
